...
function Words( {words, highlightHandler, delHandler} ) {
    ...
    return (
      <>
        ...
        <div style={{ margin: 'auto', height: '50vh', width: '50vh', overflow: 'scroll', border: '1px solid' }}>
          <table>
            <tbody>
              {reversed.map(w => <Word key={w.id} word={w} wordNumber={wordNumber++} onHighlight={() => highlightHandler(w)} showMeanings={showMeanings} delHandler={() => delHandler(w)}/>)}
            </tbody>
          </table>
        </div>
      </>
